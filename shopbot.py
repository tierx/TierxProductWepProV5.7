...
from fallback_mode import fallback_manager
try:
...
# ระบบ Heartbeat สำหรับ Production
...
async def safe_send(ctx_or_channel, content=None, **kwargs):
    ...
    # ตรวจสอบ fallback mode
    if fallback_manager.check_and_update_fallback():
        print("🔒 Fallback mode active - skipping message send")
        return None
    ...
    try:
        ...
        # ให้ fallback manager จัดการ error
        if fallback_manager.handle_discord_error(e):
            print(f"🔒 Entering fallback mode due to: {e}")
            return None
...
async def safe_reaction(message, emoji):
    """เพิ่ม reaction แบบปลอดภัย"""
    # ตรวจสอบ fallback mode
    if fallback_manager.check_and_update_fallback():
        print("🔒 Fallback mode active - skipping reaction")
        return False
        
    try:
        await message.add_reaction(emoji)
        print(f"💗 Added reaction successfully")
        return True
    except discord.HTTPException as e:
        # ให้ fallback manager จัดการ error
        if fallback_manager.handle_discord_error(e):
            print(f"🔒 Entering fallback mode due to reaction error: {e}")
            return False
...
async def heartbeat_task():
    """อัปเดต heartbeat ทุก 2 นาที"""
    try:
        heartbeat.update_heartbeat()
        print(f"💓 Heartbeat updated: {datetime.now().strftime('%H:%M:%S')}")
    except Exception as e:
...
async def on_ready():
    ...
    # เริ่ม heartbeat system สำหรับ Production
    if is_render and not heartbeat_task.is_running():
        heartbeat_task.start()
        print("💓 เริ่มระบบ heartbeat สำหรับ Production")
...
async def on_message(message):
    ...
    if message.channel.id == TARGET_CHANNEL_ID:
        ...
        try:
            # เพิ่มอีโมจิ 💗 ให้ข้อความ (ใช้ safe_reaction)
            reaction_success = await safe_reaction(message, "💗")
            
            # หากไม่สามารถเพิ่ม reaction ได้และอยู่ใน fallback mode ให้หยุด
            if not reaction_success and fallback_manager.is_fallback_mode:
                print("🔒 Stopping channel name update due to fallback mode")
                # ยังคงอัปเดต pending number ใน MongoDB แต่ไม่เปลี่ยนชื่อช่อง
                ...
                # อัปเดต pending number ใน background
                ...
                save_channel_state(current_name, actual_current_number, new_pending_number)
                print(f"💾 Background: Updated pending number to {new_pending_number}")
                return
            ...
            try:
                ...
                if fresh_pending > 0:
                    ...
                    # ตรวจสอบ fallback mode ก่อนเปลี่ยนชื่อช่อง
                    if fallback_manager.check_and_update_fallback():
                        print("🔒 Fallback mode active - skipping channel name change")
                        return
                ...
                # ให้ fallback manager จัดการ error
                if fallback_manager.handle_discord_error(e):
                    print(f"🔒 Entering fallback mode due to channel edit error: {e}")
                    return
                
                if e.status == 429:
                    print(f"⏳ Rate limited - pending number {new_pending_number} saved to MongoDB, will apply when limit clears")
                elif e.status == 403:
                    print(f"❌ Permission denied when changing channel name: {e}")
                    print("💡 Bot needs 'Manage Channels' permission or administrator role")
                else:
                    print(f"❌ Error changing channel name: {e}")
            except Exception as e:
                print(f"❌ Error applying pending change: {e}")
...
async def on_command_error(ctx, error):
    ...
    try:
        ...
        if rate_error.status == 429:
            ...
            if "cloudflare" in error_msg.lower() or "1015" in error_msg or "Access denied" in error_msg:
                print(f"🔒 Cloudflare Rate Limit detected (Error 1015) - บอท working in silent mode")
            else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if is_render:
            ...
            # สำหรับ Production: ใช้ reconnect=True เพื่อความเสถียร
            print("🔌 Starting Discord connection with auto-reconnect...")
        ...
        if e.status == 401:
            ...
            print("⚠️ Rate Limited - บอทจะรันแบบ background mode")
            if is_render:
                # สำหรับ Render: เปลี่ยนเป็น background mode
                print("🔄 Production: เปลี่ยนเป็น background mode เนื่องจาก Cloudflare Rate Limit")
                # บันทึก heartbeat ว่าบอทยังทำงานอยู่
                try:
                    heartbeat.update_heartbeat()
                except:
                    pass
                # รอแล้วทำงานต่อใน background
                delay = random.uniform(120, 180)  # รอนานขึ้น 2-3 นาที
                print(f"💤 Background mode: รอ {delay:.1f} วินาที...")
                time.sleep(delay)
                print("🔄 กลับมาลองเชื่อมต่อใหม่...")
                # กลับไปลองเชื่อมต่อใหม่
                pass
            else:
        ...
        if is_render:
            # สำหรับ Production: บันทึก heartbeat สุดท้าย
            try:
                heartbeat.update_heartbeat()
                print("💓 อัปเดต heartbeat สุดท้ายก่อนหยุด")
            except:
                pass
            
            delay = random.uniform(30, 60)
            print(f"🔄 Production: รอ {delay:.1f} วินาที ก่อน exit (Render จะ restart อัตโนมัติ)")
            time.sleep(delay)
